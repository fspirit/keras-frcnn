...
def run(options):
	...
	model_rpn.compile(optimizer=optimizer, loss=[losses_f.rpn_loss_cls(num_anchors), losses_f.rpn_loss_regr(num_anchors)])
	model_classifier.compile(optimizer=optimizer_classifier, loss=[losses_f.class_loss_cls, losses_f.class_loss_regr(len(classes_count)-1)], metrics={'dense_class_{}'.format(len(classes_count)): 'accuracy'})
